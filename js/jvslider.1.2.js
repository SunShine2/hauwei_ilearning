...
(function($){
	...
	$.fn.jvslider=function(option){
		...
		return this.each(function(){
			...
			html+='-khtml-user-select: none;user-select: none;-webkit-user-select:none; box-shadow:inset 0 0 5px rgba(0,0,0,0.75); border-top:1px solid #767676;">';
			...
			html+='box-shadow:inset 0 1px #d66464,inset 0 2px #d45d5d,inset 0 3px #d04a4a;"></div>';
			html+='</div>';
			...
			html+='position: absolute;top: -10px;left: 0;z-index: 1;-khtml-user-select: none;user-select: none;-webkit-user-select:none; border-radius:50%; border:6px solid #fff;"></div>';
			html+='</div>';
			$(this).html(html);
			...
			if(opt.vertical){
				...
				this_handle.css({'left':barPosition+'px', 'top':(-10)+'px'});
			}
            ...
            $('.slider_handle')[0].addEventListener('touchstart',function(event){
                $(this).css('background-color', opt.handleDownBackground);
                if(opt.vertical){
                    lastPosition=event.touches[0].clientY;
                }else{
                    lastPosition=event.touches[0].clientX;
                }
                $('.slider_handle')[0].addEventListener('touchmove', function(event){
                    if(opt.vertical){
                        var newB=barPosition-(event.touches[0].clientY-lastPosition);
                        if(newB<0)newB=0;
                        if(newB>opt.width)newB=opt.width;
                        lastPosition=event.touches[0].clientY;
                        barPosition=newB;
                        ...
                        var newL=barPosition+event.touches[0].clientX-lastPosition;
                        if(newL<0)newL=0;
                        if(newL>opt.width)newL=opt.width;
                        lastPosition=event.touches[0].clientX;
                        barPosition=newL;
            ...
            /*this_handle.bind('touchstart',function(e){
                $(this).css('background-color', opt.handleDownBackground);
                if(opt.vertical){
                    lastPosition=e.pageY;
                }else{
            ...
            this_handle.bind('touchend',function(){
                $(this).css('background-color', opt.handleBackground);
                $(this).unbind('mousemove');
            });
            this_handle.bind('touchend',function(){
